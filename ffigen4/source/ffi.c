...
static void
ffi_emit_fields (tree field, int public_only, int indentp)
{
      ...
      if (!public_only || (!TREE_PRIVATE (field)))
        {
          ...
          */
          ...
          else if (!DECL_NAME(field) && DECL_ARTIFICIAL (field)) 
            {
              ...
              ffi_emit_fields(TYPE_FIELDS (type), public_only, indentp);
            }
          else
            {
              tree type = TREE_TYPE (field);
...
ffi_emit_field_list (tree field, int public_only)
{
  ffi_indent ();
  fprintf (ffifile, "(");
  ffi_emit_fields(field, public_only, 0);
  fprintf (ffifile, ")");
  ffi_unindent (1);
}
