...
ffigen/include/*        header files specific to this compiler version
...
# may need to be root to write to /usr/local
shell> tar xfz /path/to/ffigen-bin-PLATFORM-gcc-GCC_VERSION.tar.gz -C /usr/local

and likewise for /usr (though this may be a poor choice) or ~ (if ~/bin is
on $PATH).
